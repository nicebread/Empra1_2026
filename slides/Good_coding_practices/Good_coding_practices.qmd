...
- AI tools create fake science that is hard to detect
...
## 
::: {.callout-box}
{{< fa people-group size=1x >}} Where can errors happen during data collection or data analysis? Be creative!
:::
...
::: {.incremental}
- Jasso (1985): Coital frequency increases with wife's age
...
- Herring (2009): Diversity increases firm revenue
...
- Munsch (2015): Marital infidelity is at 10%
  - Munsch (2018): 246 missings were miscoded as "infidelity" ("Stata `.`-problem"); with correct coding marital infidelity is at 6%
:::

::: footer
Taken from a presentation from Katrin Auspurg & Josef Brüderl. All examples are from publications in the *American Sociological Review*, considered one of the very top journals in sociology.
::: 
...
1. When you rely on random numbers (e.g., in
   simulation studies): Set a seed for reproducibility (e.g., `set.seed(123)`)
...
2. Document exact versions of all packages/ external
   ...
   - e.g., in R: Save the `sessionInfo()` of the analytical pipeline in the README file when you submit a paper, and for every revision
3. Save a snapshot of the current version/state of the statistical software
   - e.g., in `R`: packages `renv`, `checkpoint`, or `packrat`
4. The safest way: Make a docker container which contains the full computational environment (including the OS)
:::
...
**Which is an example of a sensible file naming convention?**

- [ ] `final_FINAL_v3_new_final.docx`
- [ ] `2024-03-15_Projectname_Dataset_v01.csv` 
- [ ] `Document1.docx`
- [ ] `data (copy).xlsx`
...
- For dates always use ISO dates: `YYYY-MM-DD`.
- Use paths relative to the project root instead of absolute paths:

<span style="color: darkblue;">Absolute path:<br>C:/Users/felix/Empra_Project/raw_data/study1.csv</span>
<br><br>
<span style="color: darkred;">Relative path:</span><span style="color: darkgrey;"> relative to the </span><span style="color: darkgreen;">root folder:</span><br>
<span style="color: darkgreen;">C:/Users/felix/Empra_Project/<span style="color: darkred;">raw_data/study1.csv</span><br>

- Set the project root as working directory in `R` by creating a `.Rproj` file in `C:/Users/felix/Empra_Project/`
- In scripts, only use relative paths: `read.csv("raw_data/study1.csv")`
:::
...
![](img/Directory_Organization.jpg){.img-full}
...
- **/doc**: Documents (such as PDFs, manuals, etc.) related to the project, including the data dictionary
...
- Consider to add the following information:
  ...
  - A license statement
- There might be additional READMEs in subfolders, explaining what is happening in that folder.
:::
...
- Load packages all at once at the beginning of the file
...
- Literate Programming (Donald Knuth, developer of $\LaTeX$): Computer code is for humans, not just for computers.
...
- Pair Programming
  ...
  - Nowadays, often an AI agent is the driver, while the human is an unskilled observer who has no idea what the code is actually doing.
- Golden route for increasing trust in code quality: Independent implementation (but nearly never done).

## References

::: {.smaller}
- Upcoming: Roggenkamp et al. (2026): Promoting the computational reproducibility of the social and behavioral sciences
...
- Rouder, J. N., Haaf, J. M., & Snyder, H. K. (2019). Minimizing Mistakes in Psychological Science. Advances in Methods and Practices in Psychological Science, 2(1), 3–11. [https://doi.org/10.1177/2515245918801915](https://doi.org/10.1177/2515245918801915)
- Strand, J. F. (2025). Error tight: Exercises for lab groups to prevent research mistakes. Psychological Methods, 30(2), 416–424. [https://doi.org/10.1037/met0000547](https://doi.org/10.1037/met0000547)
